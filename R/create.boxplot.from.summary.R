create.boxplot.from.summary <- function(data = NULL, x = NULL, min, max, upper, lower, middle, xlab = NULL, ylab = NULL, theme = NULL, filename = NULL, resolution = 1600) {
  ...
  plot.object <- ggplot(data = data, aes_string(x = x, ymin = min, lower = lower, middle = middle, upper = upper, ymax = max))
  plot.object <- plot.object + geom_boxplot(stat = 'identity')

  # add a theme to the plot
  if (is.null(theme)) {
    plot.object <- plot.object + plotting.general::default.boxplot.theme()
  } else {
